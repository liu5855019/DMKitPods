...
- (void)setupViews
{
    ...
    for (NSDictionary *dict in _datas) {
        ...
        if ([dict[@"image"] isKindOfClass:[NSString class]]) {
            ...
            [[SDWebImageDownloader sharedDownloader] downloadImageWithURL:[NSURL URLWithString:dict[@"imageH"]] options:0 progress:nil completed:^(UIImage * _Nullable image, NSData * _Nullable data, NSError * _Nullable error, BOOL finished) {
                if (image) {
